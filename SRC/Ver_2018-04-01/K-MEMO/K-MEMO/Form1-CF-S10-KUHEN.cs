    ...
    public partial class Form1 : Form
    {
       ...
       public Form1()
        ...
        private void label4_DoubleClick(object sender, EventArgs e)
        {
            ...
            switch (System.Environment.UserName.ToString().ToLower())
            {
                ...
                case "skojima":
                    ...
                    // どのボタンを選択したかを判断する
                    if (MessageBox.Show("今から【ＳＥ環境】に切り替えますか？ ", "【サーバ切替確認】", MessageBoxButtons.OKCancel) == DialogResult.OK)
                    {

                        MessageBox.Show("[はい] ボタンを選択しましたので【ＳＥ環境】へ切替ました！");

                        G.WK_DB_SERVER_NAME_MEMO = "SQL-DB1";
                        G.WK_DB_NAME_MEMO = "K-MEMO";
                    ...
                    {
                        MessageBox.Show("[キャンセル] ボタンを選択しましたので【ＳＥ環境】への切替を中止します");

                        G.WK_DB_SERVER_NAME_MEMO = "SQL-DB1";
                        G.WK_DB_NAME_MEMO = "K-MEMO2";

                        label4.ForeColor = Color.Black;

                    }

                    this.label4.Text = "[" + G.WK_DB_SERVER_NAME_MEMO + "] " + G.WK_DB_NAME_MEMO;
            ...
            try
            {
                ...
                switch (comboBox3.SelectedValue.ToString())
                {
                    ...
                    case "7":
                        ...
                        string selectString4 = @"SELECT ISNULL(count(name),0) FROM [sys].[objects] WHERE name LIKE REPLACE(REPLACE(LOWER ( system_user ),'\','_'),'-','') + '_' +'%'
";
                        ...
                        string WK_UMU4 = reader4.GetValue(0).ToString();
                        ...
                        if (WK_UMU4 != "0")
                        {
                            MessageBox.Show("消し残し作業用テーブルが【 " + WK_UMU4 + " 】個見つかりました" + Environment.NewLine + Environment.NewLine + "画面左上の[メンテナンス]ボタンを押すと自動消去出来ます！");
                        }
